...
from common import api, current_user, ensure_init, get_locale, goto, is_admin, load_all_problems, load_languages, page_url, render_home_button, render_page_link, render_rich_text, render_sample_cases, render_subheader, render_topbar, require_login_error, toast_safe, tr
...
def is_legacy_starter(code_text: str) -> bool:
    ...
    legacy_prefixes = ['# 在这里写 Python 3 代码', '// C++17', 'import java.util.*;', '// Node.js 20']
...
with st.container(border=True):
    ...
    with left:
        st.subheader(f'📖 {p_id} · {p_title}')
        meta = t('难度', 'Difficulty') + f": {p_diff or '-'}  |  " + t('作者', 'Author') + f': {p_author}  |  ' + t('来源', 'Source') + f': {p_source}'
        if p_tags:
...
with st.container(border=True):
    ...
    with row1:
        new_lang = st.selectbox(t('语言', 'Language'), lang_names, index=lang_names.index(cur_lang), key=pk + 'lang_widget')
    ...
    with row2:
        ...
        compare_labels = {'exact': t('精确(行对比)', 'Exact'), 'trim': t('Trim 去首尾空格', 'Trim'), 'numeric': t('数值容差', 'Numeric')}
        cur_cm = gs('cm') or 'exact'
        new_cm_label = st.selectbox(t('对比方式', 'Compare Mode'), [compare_labels[item] for item in compare_modes], index=compare_modes.index(cur_cm if cur_cm in compare_modes else 'exact'), key=pk + 'cm_widget')
        ss_set('cm', compare_modes[[compare_labels[item] for item in compare_modes].index(new_cm_label)])
    with row3:
        tl_val = st.number_input(t('时间限制 (s)', 'Time Limit (s)'), min_value=0.1, max_value=60.0, step=0.1, value=float(gs('tl') or 1.0), key=pk + 'tl_widget')
    with row4:
        ml_val = st.number_input(t('内存限制 (MB)', 'Memory Limit (MB)'), min_value=8, max_value=2048, step=8, value=int(gs('ml') or 128), key=pk + 'ml_widget')
    cur_code = st.text_area(t('代码编辑器', 'Code Editor'), value=gs('code') or '', height=420, key=pk + 'code', help=t('按一般 OJ 习惯直接写代码即可，不需要任何系统模板。', 'Write code directly like a typical OJ. No system starter template is required.'))
    ...
    with add_left:
        st.caption(t('下面是你自己的测试输入和期望输出，只用于本次调试，不会写回题库。', 'These are your own input/output cases for this debug run only and will not be saved into the problem.'))
    with add_right:
    ...
    if not custom_list:
        ...
        for idx, case in enumerate(custom_list):
            with st.expander(t(f'样例 #{idx + 1}', f'Case #{idx + 1}'), expanded=True):
                ...
                with ci:
                    case_input = st.text_area(t('输入', 'Input'), value=case.get('input', ''), height=100, key=f'{pk}custom_in_{idx}')
                with co:
                    case_output = st.text_area(t('期望输出', 'Expected Output'), value=case.get('output', ''), height=100, key=f'{pk}custom_out_{idx}')
                with cd:
                    st.caption('')
    ...
    if submit:
        if not use_samples and (not use_custom):
            ...
            st.stop()
        custom_payload_cases = [{'input': item.get('input', ''), 'expected': item.get('output', '')} for item in gs('custom_list') or [] if item.get('input') or item.get('output')]
        if use_custom and (not custom_payload_cases):
            ...
            st.stop()
        payload = {'problem_id': p_id, 'language': gs('lang'), 'code': cur_code or '', 'time_limit': float(tl_val), 'memory_limit': int(ml_val), 'compare_mode': gs('cm') or 'exact'}
        if assignment_id:
            payload['assignment_id'] = int(assignment_id)
        if exam_id:
            payload['exam_id'] = int(exam_id)
        if use_samples and samples:
            payload['samples_override'] = [{'input': item.get('input', ''), 'expected': item.get('output', '')} for item in samples]
        ...
        if c == 200 and d and d.get('data'):
            ...
            if sub.get('language_corrected_from'):
                toast_safe(t(f"已自动将语言从 {sub.get('language_corrected_from')} 调整为 {sub.get('language')} 。", f"Language auto-corrected from {sub.get('language_corrected_from')} to {sub.get('language')}."), 'warn')
            ...
            st.error(t(f"提交失败：{(d.get('msg') if d else err)} (HTTP {c})", f"Submit failed: {(d.get('msg') if d else err)} (HTTP {c})"))
    st.caption(t('提交后会直接跳转到独立评测详情页，方便第一时间查看成绩和各测试点情况。', 'After submission, the page jumps directly to the dedicated result page so you can immediately inspect the score and testcase details.'))
