"""
作业页面——教学扩展功能（与 🎓班级、📝考试 一起构成完整教学 OJ）。
= URL 参数 =
  ?id=xxx        assignment_id，可选；不传则展示"我可见的作业列表"。
= 功能 =
  - 作业详情：标题、描述、截止时间（deadline）、关联班级、题目数量、总分；
    4 个 metric：题目数 / 总分 / 我已通过 / 我得分；
  - 题目列表：每题 → 点"去做"跳 ⚖️判题器?assignment_id=xxx&id=problem_id，
    后端会把 assignment_id 一并写入 submission.assignment_id，方便统计；
  - 统计 Tab：每题当前我的提交次数、AC 状态、最高分；
  - 管理员：创建作业、编辑题目列表、绑定班级、开启/关闭（开了以后学生才看到）。
= 设计说明 =
  作业 = 固定题目集合 + 截止时间。学生在 deadline 之前做的 AC 算分；
  之后继续做题也可以但不计入作业分数。
= 与主流程关系 =
  不影响 Step1~Step6 / Advance 的评分，但是能体现"前后端整合、真实 OJ 场景"的完整性，
  答辩时间够的话可以展示"教师建班级 → 建作业 → 学生做题 → 自动出分"整条链路。
"""
import os
import sys
