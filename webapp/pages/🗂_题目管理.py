# -*- coding: utf-8 -*-
"""
题目管理 / 题库浏览页——Step1 题目管理（5分）的核心前端。
= 角色差异化（关键设计）=
  管理员 is_admin()：显示"新增 / 编辑 / 删除题目"按钮，走 POST/PUT/DELETE /api/problems；
  普通用户：只显示题目列表 + 标签 + 难度筛选 + 分页（导航文案自动变成"📚 题库浏览"而不是"🗂 题目管理"），
            每行点击"去做题"跳 ⚖️判题器 ?id=xxx。
= 功能 =
  - 支持按标题搜索、按难度（简单/中等/困难）、按 tag 多条件筛选；
  - 分页：page / page_size；
  - 批量跳转：题目详情页 📄_题目详情 / 判题 ⚖️判题器 / 最近提交 📋_提交日志。
= 对应 Step1 要求 =
  加载：GET /api/problems；校验：新增/编辑时后端校验 JSON 配置 fields/time_limit/memory_limit；
  增删改查全部齐了。
= 对应评分注意 =
  本页和 📄_题目详情.py 一起实现 Step1 的"题目配置加载、校验与增删改查"。
"""
