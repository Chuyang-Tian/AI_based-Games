# -*- coding: utf-8 -*-
"""
Streamlit 多页应用的「首页 / 入口页」脚本。
= 作用说明 =
  这是用户双击 run_oj.bat 后看到的第一个页面（浏览器默认打开 8501 端口的根路径）。
  它负责三件事：
    1. 渲染整个 OJ 的全局顶栏（配色、语言切换、登录状态、主导航跳转链接）；
    2. 从 FastAPI :5000 拉取题目/提交/用户/AI草稿等统计数据，展示首页概览卡片；
    3. 作为多页（st.multipage）入口，和 pages/ 目录下的脚本一起构成完整 UI。
= 关键依赖/数据流 =
  - 所有业务数据都通过 common.api('GET/POST', '/api/...') 向 FastAPI 发请求，
    绝不直接读写 *.db（符合 Step6 "前后端分离、禁止直连 DB" 的要求）。
  - 登录态通过 common.current_user() 从 st.session_state['oj_me'] 取，
    这个变量的刷新由 common.refresh_me() 在每个子页面初次渲染时触发。
= 与评分项的对应 =
  Step6 前端交互（5分）：本文件 + pages/ 目录下全部页面，都是 Step6 的交付内容。
  首页本身实现了"用户未登录时提示登录、已登录时展示统计卡片"的基本交互。
= 版本标签 =
  本页顶栏显示的版本号直接从 common.OJ_VERSION 读取（当前 v1.4.1），只改一处就能全站同步。
"""
