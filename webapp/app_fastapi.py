...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    ...
    if errors:
        try:
            ...
            msg = first.get('msg', '')
            first_msg = f'{loc}: {msg}'[:120]
        except Exception:
            first_msg = ''
    return _api_response(400, first_msg or '参数校验错误', None)
...
async def _login_response(user_id: int) -> Response:
    ...
    resp = _api_response(200, 'login success', data_obj)
    resp.set_cookie(
...
async def api_logout(request: Request):
    ...
    resp = _api_response(200, 'logout success')
...
async def api_update_role(request: Request, user_id: int):
    ...
    return _api_response(200, 'role updated', updated.to_public())
...
async def api_languages_create(request: Request, viewer=None):
    ...
    return _api_response(200, 'language registered', {'name': name})
...
async def api_problems_update(request: Request, problem_id: str):
    ...
    if not updated:
        return _api_response(500, '更新失败')
    return _api_response(200, 'update success', {'id': updated['id']})
...
async def api_problems_delete(request: Request, problem_id: str, viewer=None):
    ...
    return _api_response(200, 'delete success', {'id': problem_id})
...
async def api_problems_log_visibility(request: Request, problem_id: str, viewer=None):
    ...
    if not success:
        return _api_response(500, '更新失败')
    return _api_response(200, 'log visibility updated', {
        'problem_id': problem_id,
...
async def api_users_register(request: Request):
    ...
    return _api_response(200, 'register success', {
        'user_id': public.get('user_id', ''),
        'username': public.get('username', ''),
        'join_time': public.get('join_time', ''),
        'role': public.get('role', ''),
        'submit_count': int(public.get('submit_count', 0) or 0),
        'resolve_count': int(public.get('resolve_count', 0) or 0),
    })
